...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if (self.current_state.id == 'off'):
            redLight.off()
            blueLight.off()
        # system is heating and the current temperature is below the set point, the red LED should fade in and out
        if (self.current_state.id == 'heat'):
            if (temp < self.setPoint):
                redLight.pulse() # heating
            # If current temperature is at or above the set point, the red LED should be on and solid.
        ...
        if (self.current_state.id == 'cool'):
            if (temp > self.setPoint):
                blueLight.pulse() # cooling
            # If current temperature is at or below the set point, the blue LED should be on and solid.
            elif (temp <= self.setPoint):
                blueLight.on()
    ...
    def setupSerialOutput(self):
        output = str(f'{self.current_state.id},{self.getFahrenheit():0.1f},{self.setPoint}\n')
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                lcd_line_2 = f"{self.current_state.id}, S:{self.setPoint}F"
                altCounter = altCounter + 1
                if(altCounter >= 11):
